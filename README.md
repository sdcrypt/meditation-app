...
- JWT bearer authentication for administrators
...
The current authentication flow is administrator-only. Create or update a
local development administrator with:
...
| Method | Endpoint | Description |
| --- | --- | --- |
| `POST` | `/auth/login` | Obtain an admin JWT |
| `GET` | `/auth/me` | Validate a JWT and return admin details |
...
1. Move the hardcoded JWT signing secret into required environment
   configuration and rotate it.
2. Replace administrator tokens in `localStorage` with a more secure
...
5. Add normal user registration, login, password reset, and account recovery.
...
The next major product phase is authenticated user functionality:
